...
public class FunctionalApplication {
	...
	public static void main(String[] args) {
		...
		//Second we define a function that will receive the 'database' and will return a Stream of documents
		...
		Keep it in mind that a restaurant e.g. McDonald have some locals in different directions and also some records hasn't names assigned
		HINT: Remember that if the restaurant's name has spaces that mean it has more than 1 word*/

		//First we define the condition to use on filter
		Predicate<Document> oneWord = (data) -> (data.get("name").toString().split(" ").length == 1)
				&& (data.get("name").toString() != null)
				&& (!data.get("name").toString().isBlank());

		//Second we define a function that will receive the 'database' and will return a Stream of documents
		Function<ArrayList<Document>, Long> oneWordNamed = (dbRest) -> dbRest.stream()
				.filter(r -> oneWord.test(r))
				.filter(distinctByKey(p -> p.get("name")))
				.count();

		//Finally, show the result

		System.out.println("Filter #3");
		System.out.println("The number of restaurants whose name has only one word is: " + oneWordNamed.apply(dataRestaurants));
		...
		/*5: Sort all the restaurants by the grade that has received in the most recent date. If they are not receiving a grade yet (grade=Not Yet Graded), ignore them.
		* HINT: Consider create a small Restaurant object with the data that you need to archive this exercise*/
		...
		-Another stream that contains the amount of words used on the restaurant's name
		Output expected
		xxxxx has 1 word
		...
		.
		.
		If you can solve it and if in the calculator activity you don't complete/implement the division operation, this will get considered.
		*/
